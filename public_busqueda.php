...
for (reset($public_rel_tipos); $tipo_consultado = key($public_rel_tipos);
    ...
    if ($tipos_presentes[$tipo_consultado] == 1) {
        ...
        $_content->parse("content.directorio.fila");
    }
}
...
// termina tabla
$_content->parse("content.directorio");
...
// inserta en página
$_content->parse("content.parametros.logica_busqueda");
...
for ($i = 1; isset($_GET["campo$i"]) && strlen($_GET["campo$i"]) > 0; $i++) {
    ...
    if (isset($_GET["valor$i"]) && strlen($_GET["valor$i"]) > 0) {
        ...
        $_content->parse("content.parametros.lista_parametros");
...
if (isset($_GET['tipo']) && strlen($_GET['tipo']) > 0) {
    ...
    $_content->parse("content.parametros.tipo_publicacion");
...
if (isset($_GET['id_ref_bibtex']) && strlen($_GET['id_ref_bibtex'])) {
    ...
    $_content->parse("content.parametros.id_bibtex");
...
if (strlen($estados_definidos) > 0) {
    ...
    $_content->parse("content.parametros.estado_pub");
}
...
$_content->parse("content.parametros.pagina");
// finaliza tabla
if ($_SESSION['privilegios'] != INVITADO) 
	$_content->parse("content.parametros");
        ...
        while ($id_referencia = mysql_fetch_array($resultado_id)) {
            ...
            if (($pag_lim_inferior < $registros_encontrados)
                ...
                if ($tipo_cabecera != $tipo_web_actual) {
                    ...
                    $_content->parse("content.publicaciones.cabecera_tipo");
                }
                ...
                if (! ($resul_campos = mysql_query($consulta_campos, $conexion))) {
                    ...
                    if ($_SESSION['privilegios'] != INVITADO) {
                        $_content->assign("ID_REFERENCIA", $id_referencia['id_referencia']);
                        $_content->parse('content.publicaciones.lista.fila.editar');
                    }
                    ...
                    if ($id_referencia['tipo_link'] == $public_tipo_links['Interno']) {
                        ...
                        $_content->assign("LINK_PUB", "$public_dir_docs{$id_referencia['link_referencia']}");
                    
                    } else {
                        $_content->assign("LINK_PUB", $id_referencia['tipo_link']);
                    }
                    ...
                    $_content->parse("content.publicaciones.lista.fila");
...
if ($registros_encontrados == 0) {
    $_content->parse("content.resumen.resumen_negativo");
    ...
    $_content->assign('LISTA', $lista_resumen);
    $_content->parse('content.resumen.resumen_positivo');
    ...
    if ($numero_pagina != 1) {
      $_content->assign('ENLACE_PRIMERA', $cadena_busqueda);
      $_content->assign('ENLACE_ANTERIOR', $cadena_busqueda."pagina=".($numero_pagina-1));
      $_content->parse('content.resumen.primeras');
    }

    if ($numero_pagina != $ultima_pagina) {
      $_content->assign('ENLACE_SIGUIENTE', $cadena_busqueda."pagina=".($numero_pagina+1));
      $_content->assign('ENLACE_ULTIMA', $cadena_busqueda."pagina=".$ultima_pagina);
      $_content->parse('content.resumen.ultimas');
    }
    ...
    if($numero_paginas > $public_num_paginas) {
        ...
        if ($numero_pagina <= ceil($public_num_paginas/2)) {
            ...
            $_content->assign("ENLACE_PAGINA", $cadena_busqueda."pagina=".($pag_mostrar_ultima+1));
            $_content->parse("content.resumen.siguientes");
            ...
            $_content->assign("ENLACE_PAGINA", $cadena_busqueda."pagina=".($pag_mostrar_primera-1));
            $_content->parse("content.resumen.anteriores");
    ...
    for($pag = $pag_mostrar_primera; $pag <= $pag_mostrar_ultima; $pag++) {
        ...
        $_content->assign("ENLACE_PAGINA",$cadena_busqueda."pagina=".$pag);
        $_content->assign("PAGINA", $pag);

        // y vemos donde lo cargamos y como, si antes, después o si es la actual
        if($pag < $numero_pagina) {
            $_content->parse("content.resumen.anterior");
        } elseif ($pag == $numero_pagina) {
            $_content->parse("content.resumen.actual");
        } else {
            $_content->parse("content.resumen.siguiente");
        }
    }

}

$_content->assign('ENLACE_EXPORT', $cadena_busqueda);

// imprime resumen
$_content->parse('content.resumen');
